...
import { Alert, Input, message, Modal, Radio, Tabs } from "antd";
import { login, sendVerifyCode } from "@/services/user";
import { getAllUrlParams, getLink, isMobile, paramsStr } from "@/utils";
...
export default function LoginModal(props) {
  ...
  async function handleSubmit(values) {
    ...
    try {
      ...
      const { username = "", token = "", src = "default" } = data;
      if (status) {
        sendLog({
          type: "exp",
          uid: username,
    ...
    return new Promise((resolve, reject) => {
      ...
      const config = {
        ...
        validCaptchaUrl: isPre
          ? "https://pre-api.interviewdogs.com/api/userService/verifyCaptcha"
  ...
  useEffect(() => {
    if (show && temp) {
      sendLog({
        ...
        uid: "",
        spm: "login.0.0.0",
        extInfo: JSON.stringify({ source: currentSource, spmPre, id: syn }),
      });
      temp = false;
    }
  }, [show]);
  ...
  return (
    ...
    >
      {step == 1 ? (
        <div id="loginPart" className={styles.container} onClick={() => {}}>
          <div
            style={{
              position: "absolute",
              top: "30%",
